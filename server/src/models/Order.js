...
orderSchema.pre('validate', async function(next) { try { if (!this.orderId) this.orderId = `ORD-${Date.now().toString(36).toUpperCase()}-${Math.random().toString(36).slice(2,7).toUpperCase()}`; const s=await Settings.findOne({key:'upi_id'}).lean(); let cfg={delivery:49,gst:0,freeThreshold:999}; try { if(s?.value){const parsed=JSON.parse(s.value);if(parsed&&typeof parsed==='object'){cfg.delivery=Math.max(0,Number(parsed.delivery)||0);cfg.gst=Math.max(0,Number(parsed.gst)||0);cfg.freeThreshold=Math.max(0,Number(parsed.freeThreshold)||999)}} } catch {} const base=Number(this.subtotal)||0; this.shipping=base>=cfg.freeThreshold?0:cfg.delivery; this.gst=Math.round(base*cfg.gst/100*100)/100; this.total=Math.round((base+this.shipping+this.gst)*100)/100; next(); } catch(e){next(e)} });
