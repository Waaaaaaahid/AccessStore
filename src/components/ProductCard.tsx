    ...
    <Link to={`/product/${product.slug}`} className="group bg-white border border-slate-200 rounded-2xl overflow-hidden hover:shadow-[0_16px_40px_rgba(15,23,42,0.10)] hover:border-blue-200 transition-all duration-300 hover:-translate-y-1">
      <div className="relative aspect-square bg-slate-50 overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5">
          ...
          {product.discount_percentage > 0 && <span className="px-2.5 py-1 bg-slate-950 text-white text-[10px] font-bold rounded-full">-{product.discount_percentage}%</span>}
        </div>
        <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); setWished(!wished); }} className="absolute top-3 right-3 w-8 h-8 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-sm hover:shadow-md transition-all">
          <Heart className={`w-4 h-4 transition-colors ${wished ? 'fill-blue-600 text-blue-600' : 'text-slate-400'}`} />
        </button>
      </div>
      <div className="p-4">
        ...
        <button onClick={handleAdd} className="w-full py-2.5 bg-slate-950 hover:bg-blue-600 text-white text-sm font-medium rounded-lg transition-all duration-300 flex items-center justify-center gap-2 group/btn"><ShoppingCart className="w-4 h-4 transition-transform group-hover/btn:scale-110" />Add to Cart</button>
      </div>
    </Link>
  );
}
